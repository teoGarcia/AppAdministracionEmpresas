...
public class ControladorControlInventario implements ActionListener {
	
	private VistaImprimir vi;
	private PanelImprimir pi;

	private VistaControlInventario vista;
	private ControlInventarioRepository repository;
	
	public ControladorControlInventario(VistaControlInventario vista) {
		this.repository = new ControlInventarioRepository();
		...
		if(e.getSource().equals(vista.getBtnGuardar())) {
			...
			if(vista.getTxtId().getText().length() <= 0) {
				if(vista.camposVacios()) {
					...
					ControlInventarioEntity ie = new ControlInventarioEntity();
					...
					ControlInventarioEntity db = this.repository.create(ie);
			...
			// ACTUALIZAR (MODIFICAR)
				...
				if(vista.camposVacios()) {
					...
					ControlInventarioEntity db = this.repository.update(ie);
			...
			if(row >= 0) {
				...
				ControlInventarioEntity ape = repository.find(id);
			...
			Imprimir(vista.getTableMaterialesElectricos(), vista.getModelMatElectricos(), "Materiales El�ctricos");
			
		}else if(e.getSource().equals(vista.getBtnImprimirMD())) {
			
			Imprimir(vista.getTableMaterialesDiversos(), vista.getModelMatDiversos(), "Materiales, repuestos y �tiles Diversos para mantenci�n y reparaci�n");
	...
	public void LlenarTabla() {
		...
		Iterator<ControlInventarioEntity> lista = this.repository.findAll().iterator();
		...
		while(lista.hasNext()) {
			...
			ControlInventarioEntity ie = lista.next();
