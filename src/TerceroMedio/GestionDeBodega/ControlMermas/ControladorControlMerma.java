...
import TerceroMedio.GestionDeBodega.ControlMermas.Imprimir.*;
import core.Helpers;
import core.ManagerDB;
import ui.Mensejes.Mensajes;

public class ControladorControlMerma implements ActionListener {
	
	private VistaImprimir vi;
	private PanelImprimir pi;
	
	private VistaControlMerma vmm;
	private ControlMermaRepository repository;

	public ControladorControlMerma(VistaControlMerma vmm) {
		this.repository = new ControlMermaRepository();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource().equals(vmm.getBtnGuardar())) {
			...
			if(vmm.getTxtId().getText().length() <= 0) {
				if(vmm.camposVacios()) {
					ControlMermaEntity mme = new ControlMermaEntity();
			...
			if(row >= 0) {
				...
				ControlMermaEntity mme = repository.find(id);
			...
			if(row >= 0) {
				...
				Long id = Long.parseLong(String.valueOf(vmm.getModel().getValueAt(row, 0)));
				ControlMermaEntity cpe = repository.find(id);
	...
	public void LlenarTabla() {
		...
		Iterator<ControlMermaEntity> lista = this.repository.findAll().iterator();
		this.vmm.getModel().getDataVector().removeAllElements();
		this.vmm.getModel().fireTableDataChanged();
		
		while(lista.hasNext()) {
			ControlMermaEntity mme = lista.next();
